...
and not repeated here, and so are custom covariate distributions
(`var_type = "custom"`). For mediation (direct/indirect decomposition)
see
...
#>       Intervention  Risk_type    Estimate
#>             <char>     <char>       <num>
#> 1: never - natural Difference -0.02404395
#> 2: never / natural      Ratio  0.96069728
```
...
With 1 825 daily time steps this Monte Carlo loop takes a few minutes,
which is why this vignette is **precomputed**: the code above was run
once when the vignette was assembled, so its output is real while the
shipped vignette builds instantly. Set `R > 1` for bootstrap confidence
intervals (much slower again on this dataset).
...
Set `return_data = TRUE` to retrieve the simulated Monte Carlo dataset
