...
|----|:--:|:--:|
| `"binary"` | ✓ | ✓ |
| `"normal"` (untruncated Gaussian draw) | ✓ | ✓ — via `spec_model(truncate = FALSE)` |
| `"categorical"` | ✓ | ✓ |
| `"bounded normal"` (clipped to observed range) | ✓ | ✓ — this is `"normal"`’s **default** behaviour |
...
An important difference is hiding in the first two rows. In `causalMed`,
`var_type = "normal"` clips each simulated draw to the range of the
response observed in the data — that is, it behaves as `gfoRmula`’s
**bounded normal**, not as its plain normal. This is a deliberate guard
against implausible extrapolation, but it means the two packages
simulate different laws whenever the clipping actually binds. Set
`spec_model(truncate = FALSE)` to recover the untruncated Gaussian draw
and match `gfoRmula`’s `"normal"` exactly. (The numerical agreement
demonstrated below is on data where clipping essentially never binds, so
it is unaffected.)

For the remaining distributions (e.g. a biomarker that can be zero or a
treatment that is absorbing once initiated), `gfoRmula` is the
appropriate choice. However, one can use `custom_fit` and `custom_sim`
to implement custom distributions and simulations in the `spec_model`
function. This gives users the flexibility to model complex scenarios,
but it should be done with caution and careful validation, as it
requires more manual coding and is not yet supported by built-in
diagnostics.
...
#> [1] data.table_1.18.4 causalMed_0.0.5  
#> 
