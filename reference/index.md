# Package index

## All functions

- [`check_recode_param()`](https://adayim.github.io/causalMed/reference/check_recode_param.md)
  : Validate recode parameters
- [`dyn_int()`](https://adayim.github.io/causalMed/reference/dyn_int.md)
  : Define a dynamic intervention rule
- [`gformula()`](https://adayim.github.io/causalMed/reference/gformula.md)
  : Parametric G-formula for Time-varying Intervention Analyses
- [`gvhd`](https://adayim.github.io/causalMed/reference/gvhd.md) : Bone
  marrow transplant data (graft-versus-host disease)
- [`mediation()`](https://adayim.github.io/causalMed/reference/mediation.md)
  : G-formula based mediation analysis
- [`nonsurvivaldata`](https://adayim.github.io/causalMed/reference/nonsurvivaldata.md)
  : Example Dataset for a Non-Survival Outcome
- [`print(`*`<gformula>`*`)`](https://adayim.github.io/causalMed/reference/print.gformula.md)
  : Print
- [`recodes()`](https://adayim.github.io/causalMed/reference/recodes.md)
  : Define parameters for recoding
- [`spec_model()`](https://adayim.github.io/causalMed/reference/spec_model.md)
  : Model specification for G-formula
- [`summary(`*`<gformula>`*`)`](https://adayim.github.io/causalMed/reference/summary.gformula.md)
  : Summary
- [`survivaldata`](https://adayim.github.io/causalMed/reference/survivaldata.md)
  : Example Dataset for a Survival Outcome
- [`yamamurodata`](https://adayim.github.io/causalMed/reference/yamamurodata.md)
  : Yamamuro et al. (2021) Multi-Mediator Simulation Dataset
