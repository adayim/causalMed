...
dens_value <- function(model, newdt) {
  ...
  if (model$var_type == "categorical") {
    ...
    # Integer row index + match() on the level names: a mixed
    # (integer, character) cbind would coerce the row index to character
    # and depend on pred carrying canonical "1".."n" row names.
    col_ix <- match(as.character(y), colnames(pred))
...
.tmle_lag_map <- function(in_recode) {
  keep <- vapply(in_recode, is.name, logical(1))
  lapply(in_recode[keep], as.character)
}

# Validate that the recode hooks stay inside what the TMLE engine can honour.
#
# The engine reads `in_recode` only through .tmle_lag_map() (bare-name lag
# entries) and `init_recode` only through .tmle_init_vals() (constant entries)
# plus a one-shot apply_recodes() on the first-time rows; `out_recode` is not
# consumed at all. Anything outside that vocabulary would be silently dropped,
# leaving derived columns stale in the R-step evaluation and exposure-derived
# columns un-intervened in the clever-covariate densities -- a silently
# distorted TMLE. Refuse rather than mislead: error and point at "gcomp",
# whose Monte Carlo engine evaluates arbitrary recode expressions correctly.
#
# Chained exposure lags (e.g. recodes(lag2_A = lag_A)) are rejected for the
# same reason: .tmle_set_regime() intervenes only first-order exposure lags
# (source is the exposure itself), so a deeper exposure-history column would
# silently keep its observed value inside regime evaluations. Handling them
# correctly would need a per-lag init boundary (lag2_A at t = 2 is
# pre-baseline A(0), not the regime value), which the engine does not model.
# Chains of NON-exposure columns are fine: covariate history is conditioned
# on as observed, and the propagation's lag-map shift handles any depth.
.tmle_check_recodes <- function(in_recode, init_recode, out_recode, exposure) {
  if (!is.null(out_recode) && length(out_recode) > 0L) {
    stop("estimator = 'tmle' does not support out_recode (the targeted ",
         "sequential regressions never evaluate an end-of-step hook, so it ",
         "would be silently ignored). Use estimator = 'gcomp'.",
         domain = "causalMed")
  }

  bad_in <- names(in_recode)[!vapply(in_recode, is.name, logical(1))]
  if (length(bad_in) > 0L) {
    stop(sprintf(paste0(
      "estimator = 'tmle' only supports in_recode entries that copy a single ",
  ...
  bad_init <- names(init_recode)[!vapply(init_recode, function(ex)
    is.name(ex) || is.numeric(ex) || is.logical(ex), logical(1))]
  if (length(bad_init) > 0L) {
    stop(sprintf(paste0(
      "estimator = 'tmle' only supports init_recode entries that are a ",
      "constant or a single column name. These entries are expressions and ",
      "would not be honoured in the intervened regime: {%s}. Use ",
      "estimator = 'gcomp'."), paste(bad_init, collapse = ", ")),
      domain = "causalMed")
  }

  # Chained exposure lags: flag any lag column whose source chains to the
  # exposure through one or more other lag columns (depth >= 2).
  lag_map <- .tmle_lag_map(in_recode)
  chained <- character(0)
  for (nm in names(lag_map)) {
    src   <- lag_map[[nm]]
    depth <- 1L
    while (!identical(src, exposure) && src %in% names(lag_map) &&
           depth <= length(lag_map)) {
      src   <- lag_map[[src]]
      depth <- depth + 1L
    }
    if (identical(src, exposure) && depth >= 2L) chained <- c(chained, nm)
  }
...
.tmle_init_vals <- function(init_recode) {
  keep <- vapply(init_recode, function(ex) is.numeric(ex) || is.logical(ex),
                 logical(1))
  lapply(init_recode[keep], as.numeric)
}
...
# Fit the quasibinomial working regression of `resp ~ predictors` on the rows
# where resp is finite; returns a small object .tmle_qpred() can evaluate on
# any number of datasets, so a fit needed at several evaluation points (the
# R-step: EIC rows plus the backward propagation) is fitted once. Falls back
# to the mean of resp when the design is degenerate. Warnings are routed
# through the package warning collector.
.tmle_qfit <- function(resp, fit_dt, predictors, label) {
  ...
  if (sum(fit_ok) == 0L)   return(list(type = "na"))
  if (length(preds) == 0L)
    return(list(type = "mean", value = .tmle_bound(mean(resp[fit_ok]))))
...
# Bounded predictions from a .tmle_qfit() object on `pred_dt` (which may be a
# regime-modified copy).
.tmle_qpred <- function(qfit, pred_dt) {
  switch(qfit$type,
    na   = rep(NA_real_, nrow(pred_dt)),
    mean = rep(qfit$value, nrow(pred_dt)),
    glm  = .tmle_bound(as.numeric(
      predict(qfit$fit, newdata = as.data.frame(pred_dt), type = "response")
    ))
  )
}

# One-shot fit + predict (L- and Z-steps, which evaluate at a single point).
.tmle_qreg <- function(resp, fit_dt, pred_dt, predictors, label) {
  .tmle_qpred(.tmle_qfit(resp, fit_dt, predictors, label), pred_dt)
}
...
.tmle_fluctuate <- function(resp, qbar, H, label,
  ...
  eps <- tryCatch({
    # RMS-scale the clever-covariate weights before the weighted GLM (as
    # medltmle does with scale(w, center = FALSE)). A common positive rescaling
    # of GLM weights leaves the fitted coefficient unchanged in exact
    # arithmetic -- the weighted score equations are homogeneous in the weights
    # -- so eps is identical; this is purely numerical conditioning that guards
    # the 1/cum.g blow-ups from tripping glm()'s convergence.
    wv <- H[use]
    wv <- wv / sqrt(sum(wv^2) / max(1L, length(wv) - 1L))
    dfl <- data.frame(y = .tmle_bound(resp[use]),
                      off = qlogis(.tmle_bound(qbar[use])),
                      w = wv)
    # weights = dfl$w (not a bare `w`) so the column needs no
    # utils::globalVariables() entry.
    fit <- run_withwarning_collect(
      glm(y ~ 1 + offset(off), data = dfl, weights = dfl$w,
...
tmle_natural_mediation <- function(data,
  ...
  # NAs in any modelled column would be silently dropped by glm()'s na.action
  # in both the nuisance fits and the sequential regressions, shrinking the
  # effective sample and miscalibrating the clever-covariate weights without
  # any visible signal. Surface it.
  model_cols <- unique(unlist(lapply(models, function(m)
    all.vars(formula(m$call)))))
  model_cols <- intersect(model_cols, names(data))
  na_cols <- model_cols[vapply(model_cols, function(v) anyNA(data[[v]]),
                               logical(1))]
  if (length(na_cols) > 0L) {
    causalmed_env$warning <- c(
      causalmed_env$warning,
      sprintf(paste0("estimator = 'tmle': NA values present in modelled ",
                     "column(s) {%s}. Rows with NA are dropped from the ",
                     "nuisance fits and the targeted regressions, which can ",
                     "bias the estimate and its influence-curve CI. Ensure ",
                     "lagged/derived columns are initialised by init_recode."),
  ...
  dens_by_regime <- function(model) {
    d0 <- numeric(nrow(data))
    d1 <- numeric(nrow(data))
  ...
  # ---- run functional J^{a, a'} ---------------------------------------
  run_jaa <- function(a, ap) {
    a_chr <- as.character(a)
    ap_chr <- as.character(ap)
    ...
    for (ti in rev(seq_len(Tn))) {
      ...
      # mapping) to produce the next backward response: one fit, two
      # evaluation points.
      qfitR <- .tmle_qfit(qZs, rows, pred_R,
                          label = sprintf("R-step t=%s", time_seq[ti]))
      qR    <- .tmle_qpred(qfitR, nd_a)
      ...
      # is_first = FALSE always: this branch only runs for ti >= 2, so the
      # evaluated fit is a time-t >= 2 function whose exposure lag means
      # A(t-1) -- under the intervened regime that is `a`, never the
      # init_recode baseline (which applies only to time-1 functions).
      ndp <- .tmle_set_regime(ndp, exposure, exp_lags, a,
                              is_first = FALSE, init_vals)
      qR_prev <- .tmle_bound(plogis(qlogis(.tmle_qpred(qfitR, ndp)) + epsR))
  ...
  res <- lapply(combos, function(cc) run_jaa(cc[1], cc[2]))
