...
#' outcome is realised once, at the end of follow-up.
#'
#' The generating parameters are, with \eqn{V \sim N(0, 1)} and all lagged terms
#' set to zero at \eqn{t = 0}:
#'
#' \preformatted{
...
#' }
#'
#' Both outcomes share that same \eqn{\pi}: \code{Y_bin} is
#' \eqn{\mathrm{Bernoulli}(\pi)} and \code{Y_cont} is
#' \eqn{\mathrm{Beta}(50\pi,\, 50(1-\pi))}. \code{Y_cont} therefore lies
#' strictly in \eqn{(0, 1)}; fitting it with \code{var_type = "normal"} is a
#' working approximation, not the generating distribution.
#'
#' The confounders \eqn{L1} and \eqn{L2} depend on current exposure, so
...
#'
#' \strong{Large-sample true values} (risk differences, computed under the same
#' permuted-pool construction the package uses for
#' \code{mediation_type = "I"}, at 200,000 subjects):
#'
#' \tabular{lr}{
...
#' deliberately non-zero, so that the decomposition and its non-additivity are
#' both visible in a worked example.
#'
...
#'
#' @format A data frame with 10,223 rows and 11 variables, in long at-risk
#'   format (one row per subject per time point while still at risk; 3000
#'   subjects, 1822 events, 283 censored, no missing values):
#' \describe{
#'   \item{id}{Unique subject identifier (1-3000).}
...
#'   \item{A}{Time-varying binary exposure.}
#'   \item{M}{Time-varying binary mediator.}
#'   \item{Y}{Event indicator at this time point (1 = event, 0 = still at
#'     risk).}
#'   \item{C}{Loss-to-follow-up indicator at this time point (1 = censored).
#'     Censoring is drawn only among subjects who did not have the event at
#'     that time, so \code{Y} and \code{C} are never both 1.}
...
#' }
#'
#' The hazard carries no time trend. The confounder \eqn{L} depends on current
#' exposure, so exposure-affected confounding of the mediator-outcome
#' relationship is present.
#'
#' \strong{Large-sample true values} (cumulative risk by the fifth period,
#' computed under the same permuted-pool construction the package uses for
#' \code{mediation_type = "I"}, at 200,000 subjects):
#'
#' \tabular{lr}{
#'   Risk under no exposure \tab 0.39 \cr
#'   Risk under exposure \tab 0.79 \cr
...
#' subjects, 1822 have the event, 283 are censored and the remaining 895 reach
