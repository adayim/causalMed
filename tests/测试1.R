...
data1 <- data1 %>%
  group_by(id_num) %>%  
  mutate(
    lag1_treat = lag(treat, 1),  # treat 的滞后 1 期
    lag1_cov1 = lag(cov1, 1),    # cov1 的滞后 1 期
    cumavg_treat = cummean(ifelse(is.na(treat), 0, treat)),  # treat 的累积平均值
...
res2<- gformula(data=data1,
               ...
               in_recode = NULL,
               out_recode = c("cumavg_treat=cumavg_treat+treat","cumavg_cov1=cumavg_cov1+cov1","cov1=cov1","time_f=time_f"),
               return_fitted = T,
               mc_sample = 250000,
               return_data = T,
               R = 50,
...
res<- gfoRmula::gformula(obs_data = data2,
                                    ...
                                    outcome_name = 'outcome', covtypes = c('binary', 'normal', 'categorical time'),
...
mod2<-spec_model(treat ~ cov3 + time_f,var_type= "binomial",mod_type = "exposure")
mod3<-spec_model(outcome ~  treat + cov1 +  cov3,var_type = "binomial",mod_type = "outcome")
models1<-list(mod1,mod2,mod3)
res2 <- gformula(data=data1,
               id_var='id_num',
               base_vars="cov3",
...
res <- gfoRmula::gformula(obs_data = data2,
                                     ...
                                     time_name = 'time', covnames = c('cov1', 'treat', 'time_f'),
                                     outcome_name = 'outcome', covtypes = c('binary', 'normal', 'categorical time'),
                                     covparams = list(covmodels = c(cov1 ~  cov3 + time_f,
                                                                    treat ~ cov3 + time_f,
                                                                    NA)), 
                                     ymodel = ymodel, 
...
for (seed_num in 1:100) {
mod1<-spec_model(cov1 ~ lag1_treat + lag1_cov1  +cov3 + time_f,var_type  = "binomial",mod_type = "covariate")
mod2<-spec_model(treat ~ lag1_treat + cov3 + time_f,var_type= "binomial",mod_type = "exposure")
mod3<-spec_model(outcome ~  treat + cov1 + lag1_cov1 + cov3,var_type = "binomial",mod_type = "outcome")
models1<-list(mod1,mod2,mod3)
res2 <- gformula(data=data1,
               id_var='id_num',
               ...
               in_recode = NULL,
               out_recode = c("treat=treat","cov1=cov1","lag1_treat=treat","lag1_cov1=cov1"),
               return_fitted = T,
               mc_sample = 2501,
               return_data = T,
               R = 50,
...
res <- gfoRmula::gformula(obs_data = data2,
                                     outcome_type = 'binary_eof', id = 'id_num',
                                     time_name = 'time', covnames = c('cov1', 'treat'),
                                     outcome_name = 'outcome', covtypes = c('binary', 'normal'),
                                     covparams = list(covmodels = c(cov1 ~  cov3,
                                                                    treat ~ cov3
                                     )), 
                                     ymodel = ymodel, 
                                     intervention1.treat =list(gfoRmula::static, c(1,1,1,1,1,1,1)),
