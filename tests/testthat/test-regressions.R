...
testthat::test_that("default exposure regimes reproduce the pre-regime-feature estimates exactly", {
  # Bit-for-bit (tolerance = 0) holds only on the platform and BLAS the fixture
  # was generated on: last-bit differences in the glm fits elsewhere can flip a
  # binary Monte Carlo draw although the code is unchanged. Skipped on CRAN and
  # CI for that reason; it guards local refactors of the engine.
  testthat::skip_on_cran()
  testthat::skip_on_ci()
  # exposure_regime = 1 / reference_regime = 0 (the defaults added 2026-09-08)
  # must be the same computation as the previously hardcoded always/never
  # pair: same RNG order, same pool keys, same scalar values reaching
  # simulate_data(). Fixture generated by fixtures/make-regime-baseline.R
  # from the code BEFORE the change.
  data("nonsurvivaldata", package = "causalMed")
  baseline <- readRDS(testthat::test_path("fixtures", "regime-baseline.rds"))
  ...
  for (mtype in c("I", "N")) {
    ...
    got_es <- setNames(fit$effect_size$Est, fit$effect_size$Intervention)
    got_rd <- setNames(fit$estimate$RD, fit$estimate$Effect)
    want   <- baseline[[mtype]]
    testthat::expect_identical(names(got_es), names(want$effect_size),
                               label = paste0("intervention names [", mtype, "]"))
    testthat::expect_equal(got_es, want$effect_size, tolerance = 0,
                           label = paste0("effect_size$Est [", mtype, "]"))
