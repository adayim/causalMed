...
data2$time0<-as.numeric(data2$time-1)#######
...
for (seed_num in 130600:130650) {
  mod1<-spec_model(treat ~ lag1_treat + cumavg_cov1 +cov3 ,var_type= "binomial",mod_type = "exposure")
  mod2<-spec_model(cov1 ~ treat + lag1_cov1  +cov3 ,var_type  = "binomial",mod_type = "covariate")
  ...
  res2<- gformula(data=data1,
                             ...
                             ref_int = 1,
                             init_recode = recodes(lag1_treat = 0,lag1_cov1=0,cumavg_cov1=0,cumavg_treat=0),
                             in_recode = recodes(lag1_treat = treat,lag1_cov1=cov1,
                                           cumavg_cov1=cummean(ifelse(is.na(cov1),0,cov1)),
                                           cumavg_treat=cummean(ifelse(is.na(treat), 0, treat))),
                             out_recode = NULL,
                             return_fitted = F,
  ...
  res<- gfoRmula::gformula(obs_data = data2,
                           ...
                           time_name = 'time0', covnames = c('treat','cov1'),################3
                           outcome_name = 'outcome', covtypes = c('binary', 'binary'),
                           covparams = list(covmodels = c(treat ~ lag1_treat + cumavg_cov1 +cov3,
  ...
  covn<- ifelse(res2$effect_size[Intervention=="natural",norm_lcl]<=result_dt1[[1]] & res2$effect_size[Intervention=="natural",norm_ucl]>=result_dt1[[1]],1,0)
  covi<- ifelse(res2$effect_size[Intervention=="always",norm_lcl]<=result_dt1[[2]] & res2$effect_size[Intervention=="always",norm_ucl]>=result_dt1[[2]],1,0)
  
  err_n<-(cs_natural - bz_natural)
  err_i <- (cs_intervention - bz_intervention)
