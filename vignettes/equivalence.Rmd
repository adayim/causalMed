...
for (seed_num in 130600:130650) {
  ...
  res2<- causalMed::gformula(data=data1,
                  id_var='id_num',
                  base_vars="cov3",
                  exposure="treat",
                  time_var="time",
  ...
  res<- gfoRmula::gformula(obs_data = data2,
                           ...
                           covparams = list(covmodels = c(cov1 ~ treat + lag1_cov1 + cov3 ,
...
knitr::kable(summary_b, caption = "Summary statistics across seeds (full run).")
```
...
mod1<-spec_model(L ~ V+lag1_L+time,var_type= "normal",mod_type = "covariate")
mod2<-spec_model(A ~  V+lag1_A +lag1_L+L+time,var_type= "binomial",mod_type = "exposure")
mod3<-spec_model(Y ~ lag1_A +A + L+lag1_L +time,var_type = "binomial",mod_type = "survival")
...
for (seed_num in 500:510) {
  res2<- causalMed::gformula(data=survivaldata,
                  id_var='id',
                  base_vars="V",
                  exposure="A",
                  time_var="time",
                  ...
                  init_recode = c("lag1_L = 0","lag1_A=0"),
                  in_recode =  c("lag1_L=L","lag1_A=A"),
                  out_recode =NULL,
                  return_fitted = T,
                  mc_sample = 50000,
                  return_data = T,
...
knitr::kable(summary_s, caption = "Summary statistics across seeds (full run).")
